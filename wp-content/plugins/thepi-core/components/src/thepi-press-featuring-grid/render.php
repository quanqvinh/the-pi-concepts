...
/**
 * Server-side rendering for the Thepi Press Featuring Grid block.
 */
...
if (!$query->have_posts()) : ?>
	<p <?php echo get_block_wrapper_attributes([
				'class' => 'no-press-featuring-message'
			]); ?>>No press featuring found.</p>
<?php else: ?>
	<div <?php echo get_block_wrapper_attributes([
					'class' => 'thepi-press-featuring-container',
		...
		<div class="thepi-press-featuring-grid" style="<?php echo $inline_style ?>">
		...
		<?php if ($show_more_enabled && $total_count > $initial_count) : ?>
			...
			<script>
				(function() {
					...
					const grid = document.querySelector('.thepi-press-featuring-grid');
					if (!btn || !grid) return;
					...
					btn.addEventListener('click', function() {
						...
						// Use the WP REST API endpoint for loading more press featuring
						fetch(`/wp-json/thepi/v1/press-featuring/show-more?limit=${eachTime}&offset=${loaded}`)
							...
							.then(function(posts) {
								posts.forEach(function(post) {
									...
									let thumb = '';
									if (post.thumbnail) {
										thumb = `<div class="thepi-press-featuring-thumb"><img src="${post.thumbnail.replace(/"/g, '&quot;')}" alt="${(post.title || '').replace(/"/g, '&quot;')}" /></div>`;
									}
									const title = `<h3 class="thepi-press-featuring-title">${post.title ? post.title.replace(/</g, "&lt;").replace(/>/g, "&gt;") : ''}</h3>`;
									const subtitle = post.subtitle ? `<p class="thepi-press-featuring-subtitle">${post.subtitle.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</p>` : '';
